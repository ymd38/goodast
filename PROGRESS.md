...
- フェーズ: **PoC Phase 1**
- 作業ブランチ: `main`（**PR #21 まで マージ済み**・ダッシュボード frontend 完了）
- **UI一気通貫フロー v1（コア4画面）: 実装完了・PR 4本オープン中**（2026-07-07）— サイト登録＋所有確認ガイド（`/sites/new`）→ スキャン設定ウィザード（`/sites/[id]/scan`・プリセット light/standard/deep）→ 進捗ポーリング→結果レポート（`/scans/[id]`・スコア＋重大度色 findings＋専門家導線）。スタック PR: **#25**（設計 docs・base main）← **#26**（PR-A 登録）← **#27**（PR-B ウィザード）← **#28**（PR-C 結果）。**マージ順: #25→#26→#27→#28（各マージ後に次の base を付け替え・squash）**。web ゲート: 100 tests・カバレッジ 100/100/100・lint/type-check clean。最終ブランチレビュー済み（Important 1件=ポーリング失敗時の無限スピナー は修正済み）。実装の教訓（auto-import 実解決テスト規約・type-check 必須化 等）は `MEMORY.md` 2026-07-07 エントリ参照
- **nuclei-templates 固定版取得の配線: 完了**（ブランチ `feat/nuclei-templates-pinning`・**PR 未作成**）— 固定 tag（既定 v10.4.5）取得を `make`／`setup` に配線、worker 起動時に `templates.Verify` で版突合し fail-fast、SDK は `NUCLEI_TEMPLATES_DIR` を native 解決。企画書 §12・ADR-0002 持ち越しを解消。詳細は下記「ADR-0002 の持ち越し / 留意点」§
- **①正式対応 + ②ドリフト対策: 完了・マージ済み**（PR #23 → #22 → main）— スキャンプリセット（軽量/標準/詳細）導入でタグ・タイムアウトのハードコードを撤廃、`summary_json` を共有型 `jobs.ScanSummary` で構造的に固定。migration 000006（`scans.preset`）追加。全モジュール build/lint/unit green（jobs・engine unit 100%）、api scan/handler/report 結合テスト実 DB PASS、nuclei integration は compile 確認（テンプレ環境要）。詳細は下記「手動 E2E で判明した課題」§
...
> web スキャフォールド〜ダッシュボードは #19〜#21 で完了・マージ済み。API→worker 一気通貫の
> 実 SDK スキャンも手動 E2E（Juice Shop）で確認済み（スコア100・Info13 がダッシュボード描画）。

- **次タスク候補（backend セッション）**:
  1. ~~**①の正式対応**~~ **完了**（ブランチ `feat/scan-preset-and-summary-contract`・PR 未作成）。残: PR 作成 → レビュー → main マージ
  2. **findings ステータス更新**（false_positive / fixed マーク）等の残 API
- **次タスク候補（frontend セッション）**:
  1. ~~サイト登録画面~~ / ~~スキャン設定ウィザード＋進捗表示~~ / ~~結果レポート画面~~ **完了**（UI一気通貫 v1・PR #26/#27/#28 オープン中。残: レビュー→マージ→base 付け替え）
  2. **診断履歴画面**（`GET /sites/:id/scans`）— v1 スコープ外・未着手
  3. **a11y バッチ**（ErrorNotice atom 抽出＋role="alert"/aria-live・プリセットカード aria-pressed）— 最終レビュー follow-up
- **未検証で残っている実走確認**:
