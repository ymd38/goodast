...
- フェーズ: **PoC Phase 1**
- 作業ブランチ: `main`（**#31 まで マージ済み**・UI一気通貫フロー完成・手動 E2E 検証済み）
...
> UI一気通貫（登録→ウィザード→進捗→結果）は #31 まで main マージ済み・手動 E2E 完走確認済み。
> オープン中の PR は無し。以下は未着手・未検証の残タスク（PoC 仕上げ〜Public 化に向けて）。

- **次タスク候補（backend セッション）**:
  1. **river `max_attempts` の見直し**（現状 25 のまま）— タイムアウトしたスキャンが最大25回×プリセット時間リトライし続ける（E2E で確認）。`EnqueueScan` の `InsertTx` は opts `nil`＝river 既定 25。スキャン用に小さく（例3）＋ `context deadline exceeded` は恒久エラー扱いで即 failed に、を推奨（今セッションで rate/timeout は調整済み・max_attempts は**未対応で持ち越し**）
  2. **findings ステータス更新**（false_positive / fixed マーク）等の残 API
  3. **Dockerfile ×3**（api / worker / web）— 未作成。compose の api/worker/web は `profiles: [app]` で隔離中。揃えば `docker compose --profile app up` で有効化（Public 化・デプロイに必要）
...
- **Public 化の条件**（PoC 完了後・下記ロードマップ §Public 化）: LICENSE / SECURITY.md 整備 → `gh repo edit ... --visibility public`
