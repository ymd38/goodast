...
- フェーズ: **PoC Phase 1**（main は #35 までマージ済み）／**Phase 2 探索コア**（作業ブランチ `feat/katana-discovery`・未マージ）
- **Katana 探索（Discovery）コア: 実装完了**（作業ブランチ `feat/katana-discovery`。設計 spec: `docs/superpowers/specs/2026-07-23-katana-discovery-design.md`）:
  - クロールエンジンとして **Katana Go SDK** を独立 `Crawler`（`worker/internal/engine/discovery/katana`）に隔離採用（ZAPは未確定オプションのまま据え置き）
  - `engine` 純粋層に `Crawler` interface・`CrawlPlan`・`CrawlResult`・`DangerousPathRegexes`・`TargetsOrBase`・動的タイムアウト `ScanTimeout` を追加（unit 100%）
  - `scanjob/worker.go` を **crawl段 → 動的タイムアウト算出 → scan段** の二段オーケストレーションに配線。DI 配線・preset 別クロール上限（light=無効/standard=浅い/deep=広い）も追加
  - 発見した GET 到達 URL 群を `ScanRequest.Targets` で既存 Nuclei 診断に流し、診断対象を「単一 URL」から「発見 URL 群」へ拡張（Nuclei 側ガード・W3・認証注入は不変・加算のみ）
  - 発見 URL 数・フォーム数を `jobs.ScanSummary.Discovery` に追加（`omitempty`・前方互換）
  - **今回やらない（次タスク）**: フォームファジング（spec #30 スキャン段②）、`discovered_endpoints` テーブル＋ `GET /scans/:id/endpoints` API、発見エンドポイント一覧 UI（frontend 別セッション）
  - Makefile: `cover` の除外に `engine/discovery/katana` を追加（katana アダプタは integration テストで検証・unit coverage 対象外）、`discovery-scan` ターゲット追加（`TestKatanaCrawl*`・要 `make juiceshop-up`）
...
- **Katana 探索（`feat/katana-discovery`・未マージ）の残タスク**:
  1. **`make juiceshop-up` → `make discovery-scan` の実走 PASS 確認**（ローカル。`TestKatanaCrawlDiscoversWithinScope` / `TestKatanaCrawlRespectsMaxURLs`）
  2. PR 作成・main へマージ
  3. **次イテレーション**: フォームファジング（spec #30 スキャン段②）、`discovered_endpoints` テーブル＋発見エンドポイント一覧 API、frontend（探索件数・発見エンドポイントの可視化）
