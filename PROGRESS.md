...
- **①正式対応 + ②ドリフト対策: 完了**（ブランチ `feat/scan-preset-and-summary-contract`・**PR 未作成**）— スキャンプリセット（軽量/標準/詳細）導入でタグ・タイムアウトのハードコードを撤廃、`summary_json` を共有型 `jobs.ScanSummary` で構造的に固定。migration 000006（`scans.preset`）追加。全モジュール build/lint/unit green（jobs・engine unit 100%）、api scan/handler/report 結合テスト実 DB PASS、nuclei integration は compile 確認（テンプレ環境要）。詳細は下記「手動 E2E で判明した課題」§
