...
- **次タスク候補（frontend セッション）**:
  1. **サイト登録画面**（`POST /sites` ＋ 所有確認ガイド図解）
  2. **スキャン設定ウィザード**（プリセット選択 ＋ 認証持込 ＋ 危険パス除外確認）＋ 進捗表示（`GET /scans/:id` ポーリング）
  3. **結果レポート画面**（`GET /scans/:id/findings`・重大度カラー / CWE / 検出URL / 修正方法 / 専門家相談導線）
  4. **診断履歴画面**（`GET /sites/:id/scans`）
- **未検証で残っている実走確認**:
